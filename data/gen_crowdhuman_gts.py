    ...
    for i, ann_data in enumerate(anns_data):
        ...
        for i in range(len(anns)):
            if 'extra' in anns[i] and 'ignore' in anns[i]['extra'] and anns[i]['extra']['ignore'] == 1:
                continue
            x, y, w, h = anns[i]['fbox']
            # x += w / 2    # 保持 xywh 的格式，对齐 DanceTrack
            # y += h / 2
...
if __name__ == '__main__':
    data_val = "./datasets/CrowdHuman/images/val"
    label_val = "./datasets/CrowdHuman/gts/val"
    ann_val = "./datasets/CrowdHuman/annotation_val.odgt"
    data_train = "./datasets/CrowdHuman/images/train"
    label_train = "./datasets/CrowdHuman/gts/train"
