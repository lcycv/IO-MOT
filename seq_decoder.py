...
from models.id_decoder import IDDecoder
from models.trajectory_modeling import TrajectoryAugmentation
...
class SeqDecoder(nn.Module):
    ...
    def forward(
        ...
        if self.training or inference_ensemble == 0:
            ...
            pred_id_words, gt_id_words,id_gts, ap_embed, mask, history= self.net_forward(
            ...
            for ensemble_i in range(inference_ensemble):
                ...
                _pred_id_words, _ , _, _, _= self.net_forward(
    ...
    def prepare(
        ...
        for t in range(T - 1):  # the historical trajectory only contains T-1 frames
            ...
            t_boxes[t_idxs] = all_boxes[t].squeeze()
            # Append to the list:
    ...
    def net_forward(
        ...
        id_words,ap_embed = self.id_decoder(
        ...
        id_gts = format_seq["unknown"]["id_gts"]  #预测对象的id
        mask = format_seq["unknown"]["masks"]    #返回预测对象的mask
        history = format_seq["trajectory"]     #所有历史信息
        if id_gts is not None:
            h, w = id_gts.shape
            ap_embed = einops.rearrange(ap_embed, "(h w) c -> h w c", h=h, w=w)
        ...
        return id_words[None, ...], id_gt_words[None, ...] if id_gt_words is not None else None, id_gts ,ap_embed, mask, history